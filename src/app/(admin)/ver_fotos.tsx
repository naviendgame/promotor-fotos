...
export default function VerFotos() {
  ...
  function obterCategoria(foto: Foto) {
    return obterCategoriaFoto(foto);
  }
  ...
  const lojasUnicas = [
    "Todas",
    ...new Set(fotos.map((foto) => foto.lojaNome || "Loja nao informada")),
  ];
        ...
        ListHeaderComponent={
          <View>
            ...
            <ScrollView
              ...
              style={{ marginBottom: 15 }}
            >
              {CATEGORIAS_FOTO_COM_TODAS.map((categoria) => (
                <TouchableOpacity
                  key={categoria}
            ...
            >
              {STATUS_FOTO_FILTRO_OPCOES.map((status) => (
                <TouchableOpacity
                  key={status}
